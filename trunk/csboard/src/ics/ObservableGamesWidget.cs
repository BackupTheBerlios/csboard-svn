...
namespace CsBoard
{
	namespace ICS
	{
		public class ObservableGamesWidget:VBox
		{
			...
			private void AddCategoryIters(int type, TreeIter parent) {
				...
				for(int i = 0; i < names.Length; i++) {
					iters[type,i] = store.AppendValues(parent, "", 0,
									   String.Format("<b><i>{0}</i></b>", names[i]), "", "");
					if(i < names.Length - 1) {
						gamesView.ExpandRow(store.GetPath(iters[type,i]), false);
					}
				}
			}
			...
			private void AddParentIters ()
			{
				...
				gamesView.ExpandRow(store.GetPath(iter), false);
						   ...
						   TreeIter iter)
			{
				...
				if(path.Indices[1] != OTHER_CATEGORY_IDX && path.Indices[1] != LIGHTNING_CATEGORY_IDX) {
					path.Up();
							   ...
							   details)
			{
				...
				GetIter(details, out iter, out needs_expansion);

				TreeIter child = store.PrependNode(iter);
				int i = 0;
				store.SetValue(child, i++, filter_string);
				store.SetValue(child, i++, details.gameId);
				store.SetValue(child, i++, details.ToPango ());
				store.SetValue(child, i++, details.TimeDetailsAsMarkup());
				store.SetValue(child, i++, details.CategoryStr);
