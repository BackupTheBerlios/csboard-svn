...
namespace CsBoard {
	...
	public class GameAdvertisement
	{
		...
		public bool IsComputer {
			get {
				return (ti & SEEK_AD_TITLE_COMPUTER) != 0;
			}
		}

		public bool IsUnregistered {
			get {
				return (ti & SEEK_AD_TITLE_UNREGISTERED) != 0;
			}
		}

		public bool IsGM {
			get {
				return (ti & SEEK_AD_TITLE_GM) != 0 || (ti & SEEK_AD_TITLE_WGM) != 0;
			}
		}

		public bool IsIMOrFM {
			get {
				return (ti & SEEK_AD_TITLE_IM) != 0 || (ti & SEEK_AD_TITLE_WIM) != 0
					|| (ti & SEEK_AD_TITLE_FM) != 0 || (ti & SEEK_AD_TITLE_WFM) != 0;
			}
		}
			...
			while (start < end)
			  {
				  ...
				  if (start >= end)
					  break;
				  string word;
				  ParserUtils.ReadWord (buffer, ' ', ref start, end,
					    out word);
							    ...
							    int end)
		{
			...
			string str;
			ParserUtils.ReadWord (buffer, ' ', ref start, end, out str);
			...
			while (start < end)
			  {
				  ...
				  ParserUtils.ReadNameValue (buffer, ref start, end,
						 out paramname,
						 out paramvalue);
