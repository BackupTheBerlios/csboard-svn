...
namespace CsBoard {
	...
	public class GnuChess:IControl {
		...
		public event ControlHintHandler HintEvent;
			...
			if (((string) result [0]).LastIndexOf ("Cannot open file") >= 0) {
			        return;
			}
		...
		for (int i=0; i < 8; i++) 
		    ...
		    for (int j = 1; j<9; j++) {
			       string move = string.Format ("{0}{1}{2}", pos, letters[i], j);
			       Put (move);
            		       string result = output.ReadLine ();

	                       if (!result.StartsWith ("Illegal move")) {
		    	            Put ("undo");
				    correct_moves = correct_moves + string.Format ("{0}{1} ", letters[i],j);
			       }
		    }
			...
			if (WaitForOutput)
				return result;
                        ...
                        while (true) {
                                ...
                                if (String.
                                    Compare (line, "Illegal move: give me it")
                                    != 0) {
                                        result.Add (line);
