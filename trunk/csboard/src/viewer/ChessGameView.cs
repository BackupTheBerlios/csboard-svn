	...
	namespace Viewer
	{
		...
		public class ChessGameWidget:VBox
		{
			...
			public ChessGameWidget (GameViewer viewer):base ()
			{
				...
				view.VisibilityNotifyEvent += VisibilityNotify;
			...
			private void CreateTags() {
				...
				movenumberTag = tag = new TextTag(MOVENUMBER_TAG);
				...
				gametagnameTag = tag = new TextTag(GAMETAGNAME_TAG);
				tag.Weight = Pango.Weight.Bold;
				tag.RightMargin = 20;
				view.Buffer.TagTable.Add(tag);

				highlightedTag = tag = new TextTag(HIGHLIGHTED_TAG);
				tag.Weight = Pango.Weight.Bold;
			...
			private void GetItersForMove(int idx, TextBuffer buffer, out TextIter start, out TextIter end) {
				string markstr = idx.ToString();
				int len = (int) marks[markstr];
				start = buffer.GetIterAtMark(buffer.GetMark(markstr));
				end = start;
			...
			public void SetMoveIndex (int idx)
			{
				...
				if(curMoveIdx >= 0) {
					GetItersForMove(curMoveIdx, buffer, out start, out end);
					text = buffer.GetText(start, end, false);
					buffer.Delete(ref start, ref end);
					buffer.InsertWithTags(ref start, text, moveTag, taglinks[curMoveIdx] as TextTag);
				}

				if(idx >= 0) {
					GetItersForMove(idx, buffer, out start, out end);
					text = buffer.GetText(start, end, false);
					buffer.Delete(ref start, ref end);
					buffer.InsertWithTags(ref start, text, highlightedTag, taglinks[idx] as TextTag);
				}
				view.ScrollToMark(buffer.GetMark(idx.ToString()), 0, false, 0, 0);
				// jump to anchor
				curMoveIdx = idx;
			...
			private void UpdateGameDetails (TextBuffer buffer, ref TextIter iter)
			{
				...
				{
					if (i % 2 == 0) {
						buffer.InsertWithTags(ref iter, moveno.ToString(), movenumberTag);
						moveno++;
					...
					buffer.InsertWithTags(ref iter, text, moveTag, link_tag);
					marks[markstr] = text.Length;
					buffer.Insert(ref iter, " ");

					if (move.comment != null) {
						buffer.Insert(ref iter, "\n");
						buffer.InsertWithTags(ref iter, move.comment, commentTag);
						buffer.Insert(ref iter, "\n");
					}
					i++;
				}
			...
			private void SetCursorIfAppropriate(TextView view, int x, int y) {
				bool hover = false;
				...
				window.Cursor = hoveringOverLink ? handCursor : regularCursor;
			}

			private void PrintTitle (TextBuffer buffer, ref TextIter iter)
			{
				ChessGame game = viewer.CurrentGame;
				if (game == null)
					return;
				string title = String.Format("{0} vs {1}",game.White,
							     game.Black);

				buffer.CreateMark("-1", iter, true);
				buffer.InsertWithTagsByName(ref iter, title, HEADING_TAG);
				buffer.Insert(ref iter, "\n");

				Widget tagdetails = GetTagDetailsWidget();
				TextChildAnchor anchor = buffer.CreateChildAnchor(ref iter);
			...
			private Widget GetTagDetailsWidget() {
				ChessGame game = viewer.CurrentGame;
				string eco;
				GameViewer.GetOpeningName (game.GetTagValue ("ECO",""), out eco);
				string[,] values = new string[,] {
					{ Catalog.GetString("Result"), game.Result },
					{ Catalog.GetString("Date"), game.Date },
					{ Catalog.GetString("Event"), game.Event },
					{ Catalog.GetString("Site"), game.Site },
				...
				for(uint i = 0; i < count; i++) {
					...
					name.Xalign = 0;
					name.Markup = String.Format("<b>{0}</b>", values[i,0]);
			...
			void FollowIfLink (TextView view, TextIter iter)
			{
				foreach (TextTag tag in iter.Tags) {
					...
					if (move is int) {
						ShowNthMove (this, new MoveEventArgs ((int) move));
					}
				}
			}
